...
export default function ClassSelection(props) {
  ...
  const changeClass = (name) => {
    ...
    const selectedClass = props.state.classesData.find(
  ...
  const lockedBadges =
    classBadges && checkLocked(classBadges, props.state.userBadges);
  const unlockedForClass =
    lockedBadges && filterLocked(lockedBadges, classBadges);
  ...
  const classList =
    props.state.classesData &&
    props.state.classesData.map((classData, index) => {
