...
const editProfile = function (username, firstName, lastName, email, password, avatar, adventurer, userId, bio) {
  const queryStr = `
    UPDATE users
  ...
  return db.query(queryStr, [username, firstName, lastName, email, password, avatar, adventurer, bio, userId]).then();
}
